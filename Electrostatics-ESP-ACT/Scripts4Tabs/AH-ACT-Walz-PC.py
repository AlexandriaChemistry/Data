...
with open('../Charge_Models/sapt2_data.json', 'r') as json_file:
...
sapt2 = {k: v for k, v in sapt2.items() if k in allowed_pairs}
...
def dict_to_list_of_lists(data):
    ...
    for key, value in data.items():
        rows.append([key.replace("-",""), value.get("rmin", "N/A"),  f"{value.get('eelec-SAPT', 'N/A'):.1f}", f"{value.get('JC', 'N/A'):.1f}", f"{value.get('Walz2018a', 'N/A'):.1f}",  f"{value.get('GC+PGV', 'N/A'):.1f}", f"{value.get('PC+GVS', 'N/A'):.1f}"])
    return rows
...
rmsd_value1 = rmsd(sapt2, "JC", "eelec-SAPT")
rmsd_value2 = rmsd(sapt2, "GC+PGV", "eelec-SAPT")
rmsd_value3 = rmsd(sapt2, "PC+GVS", "eelec-SAPT")
rmsd_value6 = rmsd(sapt2, "Walz2018a", "eelec-SAPT")


mse_value1 = mse(sapt2, "JC", "eelec-SAPT")
mse_value2 = mse(sapt2, "GC+PGV", "eelec-SAPT")
mse_value3 = mse(sapt2, "PC+GVS", "eelec-SAPT")
mse_value6 = mse(sapt2, "Walz2018a", "eelec-SAPT")

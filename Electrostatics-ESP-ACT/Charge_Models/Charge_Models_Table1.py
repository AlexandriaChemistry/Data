...
def do_all(qm:str):
    ...
    cmd1 = f"plotxvg -f {couls} -dslegends {labels} -ls None -mk o + x -res -noshow -save legacy_coul_{qm}.pdf -panel"
    if debug:
        print(f"cmd1 = {cmd1}")
    os.system(cmd1)
    cmd2 = f"plotxvg -f {allelecs} -dslegends {labels} -ls None -mk o + x -res -noshow -save legacy_allelec_{qm}.pdf -panel"
    if debug:
        print(f"cmd2 = {cmd2}")
    os.system(cmd2)

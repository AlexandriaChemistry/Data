                    ...
                    if "Name:" in line:
                        ...
                        for compound in compounds_of_interest:
                            ...
                            if f"Name: {c2}" in line:
                                ...
                                read_data = True
                                print("Will read charges for %s from %s" %
                                      (compound, log_file))
                                break
                        if current_compound:
                            continue  
                            ...
                            if not (guanidinium_hack and "w" in atom_type ):
                                ...
                                for mypart in data[file_type][current_compound]:
                                    if mypart["type"] == atom_type:
                                        aindex += 1
                                data[file_type][current_compound].append(
                                    { "type": atom_type, "aindex": aindex, "value": acm_value }
                                )
...
def save_data_as_latex(data):
    combined_output_file = 'particle_charges3.tex'
    ...
    with open(combined_output_file, 'w') as file:
        for compound in compounds_of_interest:
            ...
            for ipart in range(maxpart):
                ...
                for method in data.keys():
                    if not compound in data[method]:
                        sys.exit("Could not find compound %s for method %s" %
                                 ( compound, method ))
                    try:
                        if method in zeta:
                        ...
                        sys.exit("Don't know how to treat charge %s for type %s" %
                                 ( mypart["value"], mypart["type"] ))
            ...
            file.write("Total")
            for method in data.keys():
                file.write(" & \\multicolumn{%d}{c}{%g}" %
                           ( log_files[method]["ncol"], round(total[method],2) ) )
            file.write("\\\\\n")
            file.write("\\hline\n")
